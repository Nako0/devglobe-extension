import { sendHeartbeat, updateStatusMessage, NetworkError, ApiError } from './heartbeat.js';
import { resetAnonymousLocation } from './geo.js';
import { HEARTBEAT_INTERVAL, ACTIVITY_TIMEOUT, OFFLINE_THRESHOLD } from './constants.js';
...
export class Tracker {
  ...
  init(apiKey: string, editor: string, shareRepo?: boolean, anonymousMode?: boolean, statusMessage?: string): void {
  ...
  async setStatus(message: string): Promise<void> {
    ...
    if (ok) {
      ...
      this.emit({ event: 'status_error', data: { message: 'Failed to update status' } });
    }
  }
  ...
  private async tick(apiKey: string): Promise<void> {
    ...
    try {
      ...
      this.pushState();
      this.emit({ event: 'heartbeat_ok', data: { today_seconds: result.todaySeconds, language: result.language } });
    } catch (e) {
      if (e instanceof NetworkError) {
        ...
        if (this.consecutiveNetErrors >= OFFLINE_THRESHOLD && !this.state.offline) {
          ...
          this.pushState();
          this.emit({ event: 'offline', data: { message: 'No network — tracking paused' } });
        }
